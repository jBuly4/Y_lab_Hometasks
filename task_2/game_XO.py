...
"""
Game board is chosen the same as showed in lectures - list of numbers for every cell.
...
computer = ""
current_player = ""
last_position = random.choice((0, 99))
...
def check_diagonals(board: List, position: int) -> bool:
    ...
    while ((column_start + count) <= column_end) and \
        ...
        diag += board[(column_start + count) + 10 * (row_start + count)]
        diag_reversed += board[(column_start + count) + 10 * (row_end - count)]
        count += 1
...
def check_vertical(board: List, position: int) -> bool:
    ...
    while (row_start + count) <= row_end:
        vertical += board[position % 10 + 10 * (row_start + count)]
        count += 1
...
def check_horizontal(board: List, position: int) -> bool:
    ...
    while (col_start + count) <= col_end:
        vertical += board[col_start + count + 10 * row]
        count += 1
...
def comp_move() -> int:
    ...
    for x in range(columns_start, columns_end + 1):
        for y in range(rows_start, rows_end + 1):
            curr_pos = x + 10 * y
            if not (PLAY_BOARD[curr_pos] in PLAYERS_MARKS):  # check if curr_pos is available
...
def player_choice(board: List, player_mark: str) -> int | bool:
    """
